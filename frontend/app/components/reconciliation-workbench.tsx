...
export function ReconciliationWorkbench() {
  ...
  const [reviewerLabel, setReviewerLabel] = useState("Finance reviewer");
  ...
  useEffect(() => {
    const savedTheme = window.localStorage.getItem("ledgerlens-theme");
  ...
  useEffect(() => { if (themeReady) { document.documentElement.dataset.theme = theme; window.localStorage.setItem("ledgerlens-theme", theme); } }, [theme, themeReady]);
  async function loadAudit(batchId: string, limit = 8) { const response = await fetch(`/api/audit-events?batch_id=${encodeURIComponent(batchId)}&limit=${limit}`); if (response.ok) setAuditEvents((await response.json()).events); }
  async function runBatch() {
    ...
    try {
      ...
      setReport(body); setSelectedId(null); setAnalysis(null); setResolutionMessage(null); setShowAllAudit(false); setState("success"); void loadAudit(body.batch_id);
  ...
  async function resolve(action: "approved" | "rejected") { if (!selected || !analysis?.advisory_id || !report) return; const response = await fetch("/api/resolutions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ batch_id: report.batch_id, source_id: selected.source_id, advisory_id: analysis.advisory_id, action, actor_label: reviewerLabel.trim() || "Finance reviewer", rationale: action === "approved" ? "Evidence reviewed; proposed follow-up accepted." : "Evidence reviewed; proposed follow-up rejected." }) }); const body = await response.json(); setResolutionMessage(response.ok ? body.message : body.detail); void loadAudit(report.batch_id, showAllAudit ? 500 : 8); }
  async function toggleAudit() { if (!report) return; if (!showAllAudit) await loadAudit(report.batch_id, 500); setShowAllAudit((visible) => !visible); }
  function reset() { setReport(null); setSelectedId(null); setAnalysis(null); setError(null); setState("idle"); }
  const metrics = [["Records processed", report ? String(report.records_processed) : "—"], ["Auto-match rate", report ? `${Math.round(report.auto_match_rate * 100)}%` : "—"], ["Verified accuracy", report ? (report.verified_matching_accuracy === null ? "Not supplied" : `${Math.round(report.verified_matching_accuracy * 100)}%`) : "—"], ["Unresolved", report ? String(report.unresolved_exceptions) : "—"], ["Engine throughput", report?.throughput_records_per_second ? `${report.throughput_records_per_second}/s` : "—"]];
  return <main className="workbench">
    <header className="topbar"><div className="brand"><img className="brand-logo" src="/ledgerlens-logo.svg" alt="LedgerLens reconciliation lens"/>LedgerLens</div><div className="topbar-actions"><p className="source-label">Synthetic data only · no live Razorpay integration</p><button className="theme-toggle" onClick={() => setTheme(theme === "light" ? "dark" : "light")} aria-label={`Switch to ${themeReady && theme === "dark" ? "light" : "dark"} mode`} aria-pressed={themeReady && theme === "dark"}>{themeReady && theme === "dark" ? "◑ Light" : "◐ Dark"}</button></div></header>
    <section className="runbar" aria-labelledby="workspace-heading"><div><p className="eyebrow">Reconciliation workspace</p><h1 id="workspace-heading">Close the batch. Keep the uncertainty visible.</h1><p className="lede">Deterministic rules reconcile the evidence. Exceptions stay open until a human reviews them.</p></div><div className="run-controls"><label className="batch-picker">Batch<select aria-label="Select reconciliation batch" value={selectedBatchId} onChange={(event) => { setSelectedBatchId(event.target.value); reset(); }}>{batches.map((batch) => <option value={batch.id} key={batch.id}>{batch.label} · {batch.record_count} records</option>)}</select></label><a className="sample-link" href="/ledgerlens-synthetic-sample.json" download>Download sample</a><label className="import-action">Import synthetic file<input aria-label="Import synthetic JSON or CSV batch" type="file" accept=".json,.csv,application/json,text/csv" onChange={importFile}/><span>Import file</span></label><button className="primary-action" onClick={runBatch} disabled={state === "loading" || !selectedBatchId}>{state === "loading" ? "Working…" : "Run reconciliation"}</button>{report && <button className="quiet-action" onClick={reset}>Reset view</button>}</div></section>
    ...
    <section className="audit-panel"><div><p className="eyebrow">Append-only audit trail</p><h2>{showAllAudit ? "Recorded workflow events" : "Latest workflow events"}</h2>{report && <button className="audit-toggle" onClick={toggleAudit}>{showAllAudit ? "Show latest" : "View audit history"}</button>}</div>{auditEvents.length ? auditEvents.map((event) => <article key={event.id}><strong>{words(event.event_type)}</strong><span>{event.entity_id}</span><time>{new Date(event.created_at).toLocaleTimeString()}</time></article>) : <p className="empty-copy">Run a batch to create auditable workflow events.</p>}</section>
  </main>;
}
